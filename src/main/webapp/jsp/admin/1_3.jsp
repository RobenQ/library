		...
		<div class="list">
			<table class="table" id="tab">
				...
				<c:forEach items="${bookTypes }" var="bookType" varStatus="status">
					<tr class="relative">
						<td class="option">${bookType.uuid }</td>
						<td class="option">${bookType.typeName }</td>
						<td class="option">${bookType.days }</td>
						<td>
							<button id="btn2" class="btn" uuid="${bookType.typeName }" onclick="xg(this)">修改</button>
							<button id="btn3" class="btn" uuid="${bookType.uuid }" onclick="sc(this)">删除</button>
