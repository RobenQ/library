...
//	var ifa = document.getElementById("ifa");
	window.location = "/library/admin/xq?uuid="+uuid;
}

function xg(btn){
	var uuid = btn.getAttribute("uuid");
	window.location = "/library/admin/xg?uuid="+uuid;
}
...
function xg2(btn){
	...
	if(name==""||author==""||republish==""||translator==""||isbn==""||booktype==""||intime==""||price==""||bookcase==""||num==""||page==""){
		...
		ajax.open("POST","/library/admin/xg2");
...
function sc(btn){
	...
	ajax.onreadystatechange=function(){
		if(ajax.readyState==4&&ajax.status==200){
			...
			if(result=='ok'){
				alert("删除成功！");
				window.location = "/library/admin/to1_1a";
			}else if(result=='login'){
				alert("你还没有登录！");
			}else if(result=='error'){
				alert("删除失败！")
	...
	ajax.open("POST","/library/admin/sc");
	ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	ajax.send("uuid="+uuid);
}